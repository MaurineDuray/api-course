...
class CurrentUserExtension implements QueryCollectionExtensionInterface, QueryItemExtensionInterface
{
    ...
    private function andWhere(QueryBuilder $queryBuilder, string $resourceClass)
    {
        ...
        // si on demande des invoices ou des customers, alors agir sur la requête pour qu'elle tienne compte du user 
        if(($resourceClass === Customer::class || $resourceClass === Invoice::class) && !$this->auth->isGranted('ROLE_ADMIN') && $user instanceof User)
        {
            $rootAlias = $queryBuilder->getRootAliases()[0]; //permet de récup l'alias de l query builder, on récup un tableau et on veut la première entrée [0]
